...
// Serve static assets when running locally or through Express
app.use(express.static(path.join(__dirname, "..", "public")));

// --- Storage Layer: In-Memory + Optional Vercel KV / Upstash Redis ---
const memoryStore = new Map();
const activeListeners = new Map(); // id -> Set of SSE callbacks

// Vercel KV / Upstash Redis support
...
async function getSession(id) {
  ...
  // If Vercel KV configured, fetch from KV
  if (kvUrl && kvToken) {
    try {
...
app.post("/api/location", async (req, res) => {
  ...
  // Broadcast to active SSE listeners on this instance
...
// Start listening if run directly as node server
